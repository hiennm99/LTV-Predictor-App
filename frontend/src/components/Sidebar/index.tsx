...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  useEffect(()=> {
    if (filters){
      ...
      console.log("check latest filters: ", latestFilter);
      if (latestFilter && latestFilter.gamePackageName != ''){
        setIsFiltered(true);
      }
    }
  },[filters]);
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 px-6 py-5.5 lg:py-6.5">
        <NavLink to="/dashboard">
          <img className="dark:hidden rounded-xl h-24 w-full" src={Logo} alt="Logo"/>
        </NavLink>
                ...
                {(handleClick, open) => {
                  return (
                    <React.Fragment>
                      ...
                      <div
                          className={`translate transform overflow-hidden ${
                            (!account || !account.isAuthenticated || open) ? '' : 'hidden'
                          }`}
                        >
                          <ul className="mt-4 mb-5.5 flex flex-col gap-2.5 pl-6">
                            <li>
