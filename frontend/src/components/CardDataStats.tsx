...
interface CardDataStatsProps {
  ...
  levelDown?: boolean;
  children: ReactNode;
  editable?: boolean; // Bật/tắt ô nhập
  value?: number; // Giá trị có thể nhập
  onChange?: (newValue: number) => void; // Hàm xử lý khi thay đổi giá trị
}
...
const CardDataStats: React.FC<CardDataStatsProps> = ({
  ...
  levelDown,
  children,
  editable = false, // Mặc định là không chỉnh sửa
  value,
  onChange,
}) => {
  return (
          ...
          <h4 className="text-title-md font-bold text-black dark:text-white">
            {editable ? (
              <input
                ...
                className="w-full rounded-lg border px-2 py-1 text-black dark:text-white dark:bg-boxdark border-stroke dark:border-strokedark"
              />
            ) : (
              total
            )}
